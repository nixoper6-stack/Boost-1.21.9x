...
#include "Hooks/Render/OverworldDimensionHook.h"
#include "Hooks/ConnectionRequestCreateHook.h"

void HookManager::init() {
	MH_Initialize();

	uintptr_t connReqCreateAddr = MemoryUtil::findSignature(
        "48 89 5C 24 18 48 89 74 24 20 57 48 83 EC 60 48 8B FA 48 8B D9 48 8B");
    RequestHook<ConnectionRequestCreateHook>(connReqCreateAddr);


	

	RequestHook<BrightnessFogColorHook>(MemoryUtil::findSignature(
        "48 89 5C 24 18 55 48 8B EC 48 83 EC 70 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 45 F8 0F 57 C0 "
        "0F 11 45 E8 48 8B 01 4C 8B CA 4C 8D 05 ?? ?? ?? ?? 48 8D 55 E8 48 8B 40 10 FF 15 ?? ?? ?? "
        "?? 90 48 8B 4D E8 48 85 C9 0F 84 AF"));
